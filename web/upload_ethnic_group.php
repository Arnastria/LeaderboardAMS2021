...
if ( isset($_POST["submit"]) and isset($_POST["uploadkey"])) {
	if ( isset($_FILES["file"])) {
		...
		if ($_FILES["file"]["error"] > 0) {
			...
			$getdata = "SELECT GroupName from ethnic_group_result where UploadKey = $uploadKey";
			...
			$fp = fopen('temp_x2z/ethnic_group_gold_standar_tester.csv', 'r');
			$y_gold = array();
			$i = 0;
			...
			$sql = "UPDATE ethnic_group_result SET `tester accuracy` = $accuracy, `tester precision` = $precision, `tester recall` = $recall, `tester f1-score` = $f1_score WHERE Uploadkey='$uploadKey'";
			...
			if(move_uploaded_file($file_loc,$folder.$final_file))
			{
				...
				$sql = "INSERT INTO ethnic_group_submission_logs(UploadKey, GroupName, filename, mime, size, updated, data, TesterAccuracy, TesterPrecision, TesterRecall, TesterF1Score) VALUES 
